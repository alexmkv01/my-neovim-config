...
vim.keymap.set('n', 'zr', 'zr', { desc = 'Reduce folding level' })
vim.keymap.set('n', 'zR', 'zR', { desc = 'Open all folds' })
vim.keymap.set('n', 'zm', 'zm', { desc = 'Increase folding level' })
...
-- Automatically enter terminal mode when entering a terminal buffer (except when quitting or cycling buffers)
vim.g.skip_terminal_insert = false
vim.api.nvim_create_autocmd('BufWinEnter', {
  pattern = 'term://*',
  group = vim.api.nvim_create_augroup('kickstart-terminal-enter', { clear = true }),
  callback = function()
    if not vim.b.terminal_quitting and not vim.g.skip_terminal_insert then
      vim.schedule(function()
        vim.cmd 'startinsert'
      end)
    end
    ...
    config = function()
      ...
      require('telescope').setup {
        ...
        pickers = {
          ...
          marks = {
            mappings = {
              i = { ['<c-d>'] = actions.delete_mark },
              n = { ['<c-d>'] = actions.delete_mark },
            },
          },
        },
        extensions = {
      ...
      vim.keymap.set('n', '<leader>gB', builtin.git_bcommits_range, { desc = '[G]it Buffer commits [R]ange' })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          if client and client.server_capabilities.documentFormattingProvider then
            -- Disable formatting for ruff_lsp as we use conform.nvim
            ...
            vim.api.nvim_create_autocmd('BufWritePre', {
              ...
              buffer = event.buf,
              callback = function()
                -- For Python files, let conform.nvim handle it
      ...
      local servers = {
        ...
        ruff_lsp = {
          init_options = {
            settings = {
              -- Disable formatting as we use conform.nvim
              format = {
                enabled = false,
              },
              lint = {
              ...
              -- Disable auto-fixes as we handle them via conform.nvim
              fixAll = false,
              organizeImports = false,
              -- Use the ruff.toml config file
              configurationPreference = 'filesystemFirst',
            },
          },
          on_attach = function(client, bufnr)
            -- Disable formatting capability for ruff_lsp
            if client.name == 'ruff_lsp' then
              client.server_capabilities.documentFormattingProvider = false
            end
          end,
  ...
  { -- You can easily change to a different colorscheme.
    ...
    config = function()
      require('gruvbox').setup {
        ...
        palette_overrides = {},
        overrides = {
          -- Use Gruvbox orange for line numbers accent
          CursorLineNr = { fg = '#d65d0e', bold = true },
        },
        dim_inactive = false,
